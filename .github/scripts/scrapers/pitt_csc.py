...
def _normalize_entry(company: str, title: str, location: str, url: str, age_text: str, notes_list: list = None) -> dict:
    ...
    if 'simplify.jobs' in url and notes:
        pass  # already has notes
...
def scrape_pitt_csc(url: str) -> list:
    ...
    for table in tables:
        ...
        for row in rows:
            ...
            entry = _normalize_entry(company, clean_title, location, url, age_text, all_notes)
            if entry["url"]:
